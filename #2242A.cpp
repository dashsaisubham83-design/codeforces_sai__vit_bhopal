Question 2 ) A bigram in a string is a pair of adjacent characters. For example, the string helloello contains 8
...
void solve() {
    ...
    // If the number of pairs we can form is greater than k, 
    // by Pigeonhole Principle, at least one bigram must repeat.
    if (pairs > k) {
        cout << "YES" << endl;
    } else {
        cout << "NO" << endl;
